...
function LottoMark({ label = "L", compact = false }: { label?: string; compact?: boolean }) {
  return (
    <div
      className={`relative flex shrink-0 items-center justify-center rounded-xl bg-white shadow-[0_10px_30px_rgba(0,0,0,0.08)] ring-1 ring-black/5 ${
        compact ? "h-10 w-10" : "h-14 w-14"
      }`}
    >
      <div className="absolute inset-1 rounded-lg bg-gradient-to-br from-sky-50 to-yellow-50" />
      <div
        className={`relative flex items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-sky-400 font-black text-white shadow-inner ${
...
function NumberBall({ value, large = false }: { value: string; large?: boolean }) {
  return (
    <span
      className={`flex shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-sky-400 to-blue-700 font-black text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.36),0_10px_18px_rgba(37,99,235,0.18)] ${
...
function PageTabs({
  ...
  return (
    <nav className="flex w-full gap-1 overflow-x-auto rounded-full border border-border bg-white p-1 shadow-[0_16px_45px_rgba(0,0,0,0.04)] sm:w-auto">
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`inline-flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-all duration-200 ${
              isActive
                ? "bg-black text-white shadow-[0_10px_28px_rgba(0,0,0,0.18)]"
...
function LottoHero() {
  return (
    <section className="relative overflow-hidden rounded-[1.6rem] border border-border bg-white shadow-[0_22px_70px_rgba(0,0,0,0.06)]">
      <div className="absolute inset-y-0 left-0 w-full bg-[radial-gradient(circle_at_15%_20%,rgba(37,99,235,0.16),transparent_28%),radial-gradient(circle_at_88%_14%,rgba(250,204,21,0.24),transparent_28%),linear-gradient(135deg,#ffffff_0%,#f7fbff_48%,#fff9dd_100%)]" />
      <div className="relative grid gap-8 p-6 sm:p-8 lg:grid-cols-[1.05fr_0.95fr] lg:p-10">
        <div className="flex flex-col justify-between">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-blue-700">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {heroBalls.map((ball) => (
              <span
                key={ball}
                className="flex h-12 min-w-12 items-center justify-center rounded-full bg-gradient-to-br from-yellow-200 via-yellow-400 to-amber-500 px-3 text-sm font-black text-blue-950 shadow-[inset_0_2px_0_rgba(255,255,255,0.58),0_14px_24px_rgba(202,138,4,0.22)]"
              >
                {ball}
              </span>
            ))}
        ...
        <div className="relative min-h-[260px] overflow-hidden rounded-[1.25rem] bg-gradient-to-br from-blue-700 via-blue-600 to-sky-500 p-5 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.22)]">
          ...
          <div className="absolute right-7 top-8 h-36 w-28 rotate-6 rounded-2xl bg-[#fff1a8] p-4 text-blue-950 shadow-[0_24px_60px_rgba(0,0,0,0.22)]">
            <div className="h-4 w-16 rounded-full bg-amber-300" />
            <div className="mt-3 h-3 w-12 rounded-full bg-yellow-200" />
            <div className="mt-5 grid grid-cols-3 gap-1.5">
              {[0, 1, 2, 3, 4, 5].map((dot) => (
                <span key={dot} className="h-5 rounded-md bg-white/80" />
              ))}
            </div>
            <div className="absolute -left-3 top-1/2 h-6 w-6 rounded-full bg-blue-600" />
            <div className="absolute -right-3 top-1/2 h-6 w-6 rounded-full bg-sky-500" />
          </div>
          <div className="relative z-10 flex h-full flex-col justify-end">
            ...
            <div className="mt-5 flex max-w-[17rem] flex-wrap gap-2">
              {["15", "43", "07", "40", "52", "29"].map((ball) => (
                <span
                  key={ball}
                  className="flex h-9 min-w-9 items-center justify-center rounded-full bg-white px-2 text-xs font-black text-blue-700"
                >
                  {ball}
                </span>
              ))}
...
function SectionHeading({
  ...
  return (
    <div>
      <div className="inline-flex items-center gap-2 rounded-md bg-blue-50 px-3 py-1.5 text-sm font-bold text-blue-700">
...
function LottoResultCard({ result }: { result: LottoResult }) {
  return (
    <article className="group overflow-hidden rounded-[1.4rem] border border-border bg-white shadow-[0_24px_80px_rgba(0,0,0,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_30px_100px_rgba(0,0,0,0.08)]">
      <div className="grid gap-0 sm:grid-cols-[0.9fr_1.1fr]">
        <div className="relative min-h-[210px] overflow-hidden bg-gradient-to-br from-yellow-200 via-yellow-300 to-amber-400 p-5">
          <div className="absolute -left-4 top-1/2 h-8 w-8 -translate-y-1/2 rounded-full bg-white" />
          <div className="absolute -right-4 top-1/2 h-8 w-8 -translate-y-1/2 rounded-full bg-white sm:bg-[#eef8ff]" />
          <div className="absolute bottom-0 right-0 h-28 w-32 rounded-tl-full bg-white/18" />
          <div className="absolute left-5 right-5 top-5 border-t border-dashed border-amber-600/25" />

          <div className="relative flex h-full flex-col justify-between">
            <div className="flex items-start justify-between gap-4 pt-4">
              <div>
                <h3 className="text-2xl font-black tracking-tight text-blue-950">{result.game}</h3>
                <p className="mt-2 max-w-[15rem] text-xs font-semibold leading-5 text-blue-950/70">
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-blue-950/60">Current Prize</p>
              <p className="mt-1 text-3xl font-black text-red-600">{result.prize}</p>
            </div>
          </div>
        </div>

        <div className="flex min-h-[210px] flex-col justify-between bg-[#eef8ff] p-5">
          <div>
          ...
          <div className="mt-6 flex items-center justify-between gap-3 border-t border-blue-100 pt-4">
            <p className="text-sm font-semibold text-secondary">{result.date}</p>
            <button className="rounded-full bg-white px-4 py-2 text-xs font-bold text-primary ring-1 ring-blue-100 transition hover:ring-blue-200">
              Details
            </button>
          </div>
        </div>
...
function DrawDateScreen() {
  return (
    <div className="space-y-6">
      <section className="rounded-[1.6rem] border border-border bg-white p-5 shadow-[0_22px_70px_rgba(0,0,0,0.06)] sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-center">
            <input
              ...
              className="h-11 rounded-full border border-border bg-bg px-4 text-sm font-semibold text-primary shadow-sm transition focus:border-neutral-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-black/5"
            />
            <input
              type="date"
      ...
      <section className="overflow-hidden rounded-[1.6rem] border border-border bg-white shadow-[0_22px_70px_rgba(0,0,0,0.06)]">
        <div className="overflow-x-auto">
          <div className="grid min-w-[520px] grid-cols-[92px_minmax(160px,1fr)_110px] gap-3 border-b border-border bg-bg px-4 py-3 text-[11px] font-bold uppercase tracking-wide text-secondary sm:min-w-0 sm:grid-cols-[1fr_1.4fr_1fr] sm:px-6">
            <div>GAME</div>
            <div>COMBINATION</div>
            <div>DATE</div>
          </div>
          ...
          <div className="divide-y divide-white bg-white">
            {drawResults.map((result, index) => (
              <div
                ...
                className="grid min-w-[520px] grid-cols-[92px_minmax(160px,1fr)_110px] items-center gap-3 bg-[#edf8ff] px-4 py-3 text-sm font-semibold text-primary transition hover:bg-blue-50 sm:min-w-0 sm:grid-cols-[1fr_1.4fr_1fr] sm:px-6"
              >
